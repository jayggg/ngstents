...
from ngsolve import (Mesh, Draw, Redraw, CoefficientFunction, IfPos, sqrt, sin, cos, exp, x, y,
                     TaskManager, L2, GridFunction, Integrate, InnerProduct)
...
wave.SetBoundaryCF(mesh.Boundaries(".*"),mesh.BoundaryCF({"square" : uex(tau)}))
...
l2error = sqrt(Integrate(InnerProduct(u-uexact, u-uexact), mesh, order=3*order))
